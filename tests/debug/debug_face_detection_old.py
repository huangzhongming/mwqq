...
# Change to backend directory for Django setup (use relative path)
backend_path = os.path.join(os.path.dirname(__file__), 'backend') if 'backend' not in os.getcwd() else '.'
...
def test_face_detection_with_debug():
    ...
    for i, face in enumerate(faces):
        ...
        text = f"{face['type']}: {face['confidence']:.2f}"
